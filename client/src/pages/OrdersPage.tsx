...
export const OrdersPage: React.FC = () => {
  ...
  const handleDeliver = async (orderId: string) => {
    if (
      !window.confirm(
        "Confirm that you have physically delivered the item to the buyer?",
      )
    )
      return;
  ...
  const handleComplete = async (orderId: string) => {
    if (
      !window.confirm(
        "Confirm that you have received and inspected the item? This releases the payment to the seller and cannot be undone!",
      )
    )
      return;
    setButtonLoadingId(orderId);
    try {
      ...
      if (res.data.success) {
        ...
        setOrders((prev) =>
          prev.map((o) =>
            o._id === orderId
              ? { ...o, status: "completed", escrowHeld: false }
              : o,
          ),
        );
      }
              ...
              >
                ...
                <div className="w-full md:max-w-xs shrink-0 flex flex-col space-y-2">
                  <div className="flex justify-between text-xs text-slate-400 dark:text-slate-500 font-bold">
                    <span
                      className={currentStage >= 1 ? "text-primary-500" : ""}
                    >
                      Paid
                    </span>
                    <span
                      className={currentStage >= 3 ? "text-indigo-500" : ""}
                    >
                      Delivered
                    </span>
                    <span className={currentStage >= 4 ? "text-green-500" : ""}>
                      Released
                    </span>
                  </div>
