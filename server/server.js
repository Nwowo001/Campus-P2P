...
io.on("connection", (socket) => {
  ...
  socket.on("send_message", async (data, callback) => {
    ...
    if (!receiverId || !text) {
      if (callback)
        callback({ success: false, error: "Receiver ID and text required" });
      return;
    }
  ...
  socket.on("check_user_status", (checkedUserId, callback) => {
    const isOnline =
      onlineUsers.has(checkedUserId) && onlineUsers.get(checkedUserId).size > 0;
  ...
  socket.on("disconnect", () => {
    console.log(
      `User disconnected: ${socket.user.name} - Socket: ${socket.id}`,
    );
