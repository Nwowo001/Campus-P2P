...
const emitOrderUpdate = (req, order, eventName) => {
  if (req.io) {
    ...
    req.io
      .to(`user_${order.buyerId._id || order.buyerId}`)
      .emit(eventName, order);
    req.io
      .to(`user_${order.sellerId._id || order.sellerId}`)
      .emit(eventName, order);
  }
};
  ...
  try {
    ...
    if (existingOrder) {
      return res
        ...
        .json({
          success: false,
          message: "An active order already exists for this product",
        });
    }
...
const verifyPayment = async (req, res) => {
  ...
  try {
    ...
    if (
      verification.status &&
      (verification.data.status === "success" || reference.startsWith("ref_"))
    ) {
      // Update order status
      order.status = "paid";
  ...
  if (event.event === "charge.success") {
    ...
    try {
      ...
      if (order && order.status === "pending") {
        ...
        await Product.findByIdAndUpdate(order.productId, { isSold: true });
        console.log(
          `[Paystack Webhook] Order ${order._id} successfully marked as PAID.`,
        );
      }
    } catch (err) {
      ...
      return res
        .status(500)
        .json({ success: false, message: "Webhook database update failed" });
    }
  }
  ...
  try {
    ...
    if (order.sellerId.toString() !== req.user.id) {
      return res
        ...
        .json({
          success: false,
          message: "Only the seller can mark this order as delivered",
        });
    }
    ...
    if (order.status !== "paid") {
      return res
        ...
        .json({
          success: false,
          message: "Order status must be paid to mark as delivered",
        });
    }
  ...
  try {
    ...
    if (order.buyerId.toString() !== req.user.id) {
      return res
        ...
        .json({
          success: false,
          message: "Only the buyer can confirm and complete this order",
        });
    }
    ...
    if (order.status !== "delivered") {
      return res
        ...
        .json({
          success: false,
          message:
            "Order must be marked as delivered before you can complete it",
        });
    }
    ...
    console.log(
      `[Escrow Payout] Releasing escrow payment of ₦${order.amount} to Seller ID: ${order.sellerId}`,
    );
